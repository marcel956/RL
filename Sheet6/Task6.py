...
# Use same evaluation as in task 5 but now loop over learning rates
...
# --- The Schedules to Test ---
schedules_to_test = ["constant", "1/n", "1/sqrt(n)"]

# This will hold the 4 lists for EACH schedule
all_results = {}
...
for schedule_name in schedules_to_test:
    ...
    for current_episode in range(0, total_episodes, chunk_size):
        ...
        current_Q, current_N = Q_learning(env, random_policy, chunk_size, gamma=0.9, schedule_type=schedule_name, Q=current_Q, N=current_N)
    ...
    end_time = time.time()
    print(f"Runtime ({schedule_name}): {end_time - start_time:.2f} seconds")

    # Save data into the master dictionary
...
colors = {"constant": "blue", "1/n": "red", "1/sqrt(n)": "green"}
